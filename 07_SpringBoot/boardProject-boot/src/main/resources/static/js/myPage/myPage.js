...
if (updateInfo != null) {
    ...
    memberNickname.addEventListener("input", () => {
        ...
        if (memberNickname.value == initNickname) { // 변경 전 닉네임이랑 입력값이 같을 경우
    ...
    memberTel.addEventListener("input", () => {
        ...
        if (memberTel.value == initTel) { // 변경 전 전화번호랑 입력값이 같을 경우
            memberTel.removeAttribute("style");
            return;
        }
        // 정규 표현식 유효성 검사
        const regEx = /^0(1[01679]|2|[3-6][1-5]|07)[1-9]\d{2,3}\d{4}$/;
    ...
    // form 태그 제출 시 유효하지 않은 값이 있으면 제출 불가 처리
    updateInfo.addEventListener("submit", (e) => {

        // 닉네임이 유효하지 않을 경우
        if (memberNickname.style.color == "red") {
            alert("닉네임이 유요하지 않습니다.");
...
// false == 기본 이미지, true == 이번 업로드 이미지

let deleteCheck = -1;
// 프로피 이미지가 새로 업로드 되거나 삭제 되었음을 나타내는 변수
// -1 == 초기값, 0 == 프로필 삭제(x버튼), 1 == 새 이미지 업로드

let originalImg; // 초기 프로필 이미지 파일 경로

if (imageInput != null) { // 화면에 imageInput이 있을 경우

    // 프로필 이미지가 출력되는 img 태그의 src 속성을 저장
    originalImg = profileImage.getAttribute("src");
    ...
    imageInput.addEventListener("change", e => {
        ...
        if (file == undefined) { // 파일을 한번 업로드 후 다음 업로드에서 취소했을 때
        ...
        if (file.size > maxSize) { // 업로드 파일 최대 크기 초과한 경우
            alert("2MB 이하의 이미지를 선택해주세요.")
            imageInput.value = "";
            // input type = "file" 태그에 대입할 수 있는 value는 "" 뿐이다
            deleteCheck = -1; // 취소 == 파일 없음 == 초기상태
