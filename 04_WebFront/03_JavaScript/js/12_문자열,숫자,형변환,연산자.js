...
document.querySelector("#btn2").addEventListener("click", (e)=>{
    ...
    // 소수점 자릿수로 표현될 수 있도록
    // 바로 다음 소숫점 자리에서 반올림해서 문자열로 반환
    console.log((3.456).toFixed(2));
    console.log((3.456).toFixed(1));
    console.log((3.456).toFixed(0));

    // Math.random() : 0.0 ~ 1.0 실수 반환
    // 버튼 클릭 시 마다 버튼 배경색 랜덤 변환

    const r = Math.floor(Math.random() * 256); // 0.0 <= x < 255
    const g = Math.floor(Math.random() * 256); // 0.0 <= x < 255
    const b = Math.floor(Math.random() * 256); // 0.0 <= x < 255
