...
-- 사번, 이름, 부서, 직급, 급여를 조회
-- (내가 했던거 : 노옹철보다 급여를 많이 받는사람을 구하고 <<(여기서부터 안됨) 그걸 출력)
-- (노옹철의 급여를 구한 후 그 급여를 서브쿼리로 비교)
...
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE -- 여기에 들어온것만 그룹으로 함 (DEPT_CODE와 같다고 같이 해주지 않음)
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_CODE, '사수' AS 구분
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_TITLE)
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_CODE, '사원' AS 구분
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_TITLE)
JOIN JOB USING(JOB_CODE)
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > (SELECT MIN (SALARY) -- 연산자는 값 하나만 비교가능
				FROM EMPLOYEE
				JOIN JOB USING(JOB_CODE)
				WHERE JOB_NAME = '과장');	
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > ANY (SELECT SALARY -- 연산자는 값 하나만 비교가능
				FROM EMPLOYEE
				JOIN JOB USING(JOB_CODE)
				WHERE JOB_NAME = '과장');	
		-- 과장(오른쪽) 기준
-- > ANY 가장 작은값보다 큰가 (220만)
-- < ANY 가장 큰값보다 작은가 (376만)		
...
WHERE (DEPT_TITLE,JOB_NAME)  = (SELECT DEPT_TITLE, JOB_NAME 
								...
								WHERE ENT_YN = 'Y'
								AND SUBSTR(EMP_NO, 8, 1) = '2');
-- ORA-00913: 값의 수가 너무 많습니다 (WHERE에 DEPT_TITLE 하나라면)
...
--					OR (ENT_YN = 'N' AND NVL(MAIN.DEPT_CODE, '1') = NVL(SUB.DEPT_CODE, '1'))
					)
ORDER BY HIRE_DATE ;
...
(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = '204')
FROM DEPARTMENT ;
...
-- 문제 원인 --
-- SELECT 해석 순서상 ORDER BY가 가장 마지막 해석이기에
-- 급여 상위 5명이라는 의도와 맞지 않는 형태로 조회

-- 해결 방법 --
-- 인라인뷰(FROM절에 작성되는 서브쿼리)를 이용해 해결
...
-- 해결 2)
-- 해결1의 조회 결과를 FROM절에 작성하여 상위 5행만 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM(SELECT EMP_NAME ,SALARY 
		FROM EMPLOYEE
		ORDER BY SALARY DESC)--> 메인 쿼리에 포함된 VIEW 생성 구문
							--> 인라인 뷰
WHERE ROWNUM <= 5;

-- * 조건절 ROWNUM 사용 시 주의사항 *
--	조건절 ROWNUM 사용 시 
...
-- ** 인라인 뷰 사용 시 컬럼명에 유의해야한다 ** 
-- 메인쿼리에서 인라인뷰의 컬럼명을 그대로 사용해야한다
