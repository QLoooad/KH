...
/* -- DCL (Data Control Language) 데이터 제어 GRANT, REVOKE
	...
	** 권한의 종류 **
	...
	2) 객체 권한 : 특정 객체를 조작할 수 있는 권한
		객체 : TAVLE, VIEW, INDEX, SQUENVE, USER 등
...
-- 0. SQL 구문 작성 형식은 일반 형석(예전 12C 버전 이전 형식)으로 변경
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

-- 1. 사용자 계정 생성
-- CREATE USER 사용자명 IDENTIFIED BY 비밀번호;

CREATE USER cgt_sample IDENTIFIED BY "sample1234";
									-- "" << 대소문자 구분
...
-- 3. 접속 권한 부여
-- GRANT 권한[, , ,...] TO 계정명;
GRANT CREATE SESSION TO cgt_sample;
...
--> 다시 관리자 권한 계정으로 전환 후 테이블 생성 관련 권한 부여
----------- 관리자 계정으로 접속 -------------------

-- 6. 테이블 생성 권한 
--	+ 객체 생성 공간 할당
GRANT CREATE TABLE TO cgt_sample;

ALTER USER cgt_sample DEFAULT TABLESPACE SYSTEM
...
SELECT * FROM KH_CGT.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
-- 권한이 없어서 다른 계정의 테이블이 보이지 않음


------------------- KH 계정으로 접속 -------------------
-- 2. SAMPLE 계정에 EMPLOYEE SELECT 권한 부여

-- 시스템 권한 부여 : GRANT 권한[,권한,...] TO 계정명;


-- 객체 권한 부여 : GRANT 권한[,권한,...] ON 객체명 TO 계정명;
GRANT SELECT ON EMPLOYEE TO CGT_SAMPLE;
...
-- 4. 선동일 삭제
DELETE FROM KH_CGT.EMPLOYEE
WHERE EMP_ID = 200;
-- ORA-01031: 권한이 불충분합니다

------------------- KH 계정으로 접속 -------------------
-- 5. SAMPLE 계정의 EMPLOYEE 테이블 조회 권한 회수
REVOKE SELECT ON EMPLOYEE FROM CGT_SAMPLE;
------------------- SAMPLE 계정으로 접속 -------------------
...
--> SAMPLE 계정에 시퀀스, 프로시져, 클러스터, 인덱스타입, 연산자
--				타입, 트리거, 테이블 생성 권한 부여
GRANT 
CREATE SEQUENCE,
CREATE PROCEDURE,
...
CREATE TYPE,
CREATE TRIGGER,
CREATE TABLE TO CGT_SAMPLE;

--> ROLE을 이용해서 간단히 부여/회수
GRANT RESOURCE TO CGT_SAMPLE;
REVOKE RESOURCE FROM CGT_SAMPLE;

-- 사용자에게 부여된 시스템 권한 조회
--SELECT * FROM DBA_SYS_PRIVS 
--WHERE GRANTEE = 'CGT_SAMPLE';
