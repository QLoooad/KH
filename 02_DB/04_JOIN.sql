...
--INNER JOIN 앞의 INNER은 기본값이기에 생략 가능
...
WHERE DEPT_CODE = DEPT_ID(+);
-- NULL = D1 ~ D9(+)
-- 왼쪽 테이블 컬럼 값과, 오른쪽 테이블 컬럼 값이 일치하지 않아도
-- 억지로 추가해라
...
-- '='(등호)를 사용하지 않는 조인문
--  지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되  는 행들을 연결하는 방식
...
-- 사원명, 주민번호, 부서명(DEPARTMENT), 직급명(JOB)을 조회하시오.
...
-- 2. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 부서명(DEPARTMENT)을 조회하시오.
...
-- 사원명, 직급명(JOB), 부서코드, 부서명(DEPARTMENT)을 조회하시오.
...
--4. 보너스포인트를 받는 직원들의 사원명, 보너스포인트, 부서명(DEPARTMENT), 근무지역명(LOCATION)을 조회하시오.
...
--5. 부서가 있는 사원의 사원명, 직급명(JOB), 부서명(DEPARTMENT), 지역명(LOCATION) 조회
SELECT EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
--WHERE DEPT_TITLE IS NOT NULL; -- 이너 조인 할때 NULL은 자동으로 제거됨

-- 6. 급여등급별(SAL_LEVEL) 최소급여(MIN_SAL)를 초과해서 받는 직원들의
--사원명, 직급명(JOB), 급여, 연봉(보너스포함)을 조회하시오.
--연봉에 보너스포인트를 적용하시오.
...
-- 사원명, 부서명, 지역명, 국가명(NATIONAL)을 조회하시오.
...
-- 9. 보너스포인트가 없는 직원들 중에서 직급코드가 J4와 J7인 직원들의 사원명, 직급명(JOB), 급여를 조회하시오.
-- 단, JOIN, IN 사용할 것
