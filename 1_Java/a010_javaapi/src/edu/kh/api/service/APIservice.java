...
public class APIservice {
	...
	//		응용		프로그래밍	접점
	//프로그래밍 언어가 이미 가지고 있는 글래스/기능/기술 을
	//사용자가 쉽게 사용할 수 있도록 제공한 것 	
...
//			if(s.equals(temp)) return false;
	...
	/** 학생 배열에서 이름 검색
	 * @param input
	 * @return 일차파는 학생 있으면 Student[], 없으면 null
	 */
	public Student[] selectName(String input) {
		//김영희
		//손흥민,김영희,박민지
		
		//	String.split(구분자) : 문자열을 구분자를 기준으로 나눠
		//							String[] 형태로 반환
		
//		String[] names = input.split(",");
		String[] names = input.split(",|/");//구분자 2개이상 , or / 기준 비교
		
		
		//검색된 학생을 저장할 배열
		Student[] result = new Student[studentList.length];
		
		int index = 0;// result 배열에 저장된 위치를 지정할 변수
		for(String n : names) {
			for(Student s : studentList) {
				if(s == null) break;
