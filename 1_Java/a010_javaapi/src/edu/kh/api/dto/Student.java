...
public class Student {
	...
	//>>>필드를 가지고 있는 자식클래스가 알맞은 형태로 재정의(오버라이딩)
...
//	Object.hashCode
	//	두 객체의 필드 값이 같다면 hashCode()도 똑같은 정수 값을 반환해야 함
	
	//	hash 함수 : 입력받은 문자열, 숫자를 
	//				특정한 길이의 문자열/숫자로 변환
	//			>>>>최대한 중복되지 않는 숫자를 만듦
	...
	//	왜 필요하나? Java 실행 시 내부에서 객체를 검색하는 속도 증가
	
	//	어떻게 작성하는가?
	//	필드 값이 같으면 항상 같은 수가 나오도록 구현
	...
	public int hashCode() {
		...
		final int PRIME = 31;//소수(곱연산 속도 빠름)
