...
<%-- 
    ...
    prefix : 접두사     태크명 앞에 작성되는 단어 <c:if>
...
<body>
    ...
    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는 JAVA 코드(if, for, while, 변수선언, 형변환)를

        스크립틀릿대신 html 태그 형식으로 태그화 하여 표준으로 제공하는 라이브러리
    ...
    <ol>
        <li>
            <a href="https://tomcat.apache.org/download-taglibs.cgi">JSTL 다운로드 페이지</a>
            <br>
            Standard-1.2.5 -> Jar Files -> impl, spec, el 다운
        </li>
        ...
        <li>
            JSTL 라이브러리를 사용하고자 하는 JSP파일 상단에
    ...
    <pre>
        ...
        1) var : 변수명(속성 key)
        2) vlaue : 대입할 값
        3) scope : page, request, session, application 중 하나
                (기본값 page)
    </pre>
    ...
    <br>
    html 태그 해석 x(escapeXml="false" 탈출)<c:out value="${temp}" escapeXml="false" />
    <br>
    html 태그 해석 o ${temp}
