...
btn1.addEventListener("click", ()=>{
    ...
    .then(resp => resp.text()) // 변수 이름 상관 X
    // resp : 응답 객체
    // resp.text() : 응답 객체 내용을 문자열로 변환 후 반환

    .then(tel => { 
        /* 비동기 요청 후 수행할 코드 */ 
        result1.innerText = tel; // 조회 결과 result1에 출력
    })
...
testSock.onmessage = e=>{
    ...
    let obj = JSON.parse(e.data); // JSON -> JS
