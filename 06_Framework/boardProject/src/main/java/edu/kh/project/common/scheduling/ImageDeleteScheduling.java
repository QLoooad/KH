...
public class ImageDeleteScheduling {
	...
	// 행동(5초) -> 10초 대기 ->행동(5초) -> 10초 대기 
	
	//@Scheduled(fixedRate = 10000)
	// 행동(5초)
	// 대기(10초) = 토탈 10초  토탈 시간을 정해두고 그 안에 행동과 대기를 동시에 체크
	
	//@Scheduled(cron = "0,30 * * * * *")// 매 분 0, 30 초 마다
	@Scheduled(cron = "0,30 * * * * *")// 매 정시 마다
	public void test() {
//		System.out.println("스케쥴러 조건 간격 실행");
		...
		File[] imagegArr = path.listFiles();
		
		// 배열 -> List로 변환
		List<File> serverImageList = Arrays.asList(imagegArr);
